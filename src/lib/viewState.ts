 ...
 * `URLSearchParams`. Today only `viewMode` is wired to the UI; the remaining fields
 * are reserved so URL-addressable filters can be added later without changing this
 * contract or re-plumbing components.
 */
...
/**
 * Derive the records to display from the raw set and the current view state. This is
 * the single seam where filtering will live: today it is a pass-through (no filters
 * implemented), but reserved `state` fields (severity/service/from/to) slot in
 * here without touching callers.
 */
export function selectLogs(records: FlatLogRecord[], _state: LogViewState): FlatLogRecord[] {
  return records;
}
